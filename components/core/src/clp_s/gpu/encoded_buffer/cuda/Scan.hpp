...
struct EncodedBufferRequest {
    ...
    std::span<ColumnDesc const> columns{};          // Column descriptors (stream-relative offsets)
...
/**
 * Scans a device-resident ERT buffer for rows matching an int64 equality filter
 * and extracts matching rows into a compacted encoded buffer.
 *
 * The function performs:
 * 1. Scans the filter column for rows where value == target_value
 * 2. Extracts and packs all requested columns for matching rows
 * 3. Returns the compacted buffer in host memory
 *
